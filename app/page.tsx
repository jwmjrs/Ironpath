...
function GeneralInformationView() {
  ...
  const armour = [
    ['Barrows sets · 70 Defence','Ahrim can reduce an enemy’s Strength; Dharok rewards fighting at low life points; Guthan can heal from its set effect. Useful chiefly as accessible niche gear rather than modern end-game armour.'],
    ['Crystal armour · 70 Defence','A ranged tank set with a charge-based set effect. It becomes more valuable when its defensive utility matches the content you are doing.'],
    ['Anima Core armour · 80 Defence','The base sets are power armour and do not rely on a four-piece set effect. Their value is dependable mid-game offensive stats and upgrade paths.'],
    ['Custom-fit trimmed masterwork · 90 Defence','A premier melee tank set. Its set effect spreads a portion of incoming damage over time, making sustained damage easier to manage.'],
    ['Achto raid armour · 90 Defence','Raid tank armour with a defensive set effect oriented around shields and defensive abilities. Best for roles that prioritise survival and team support.'],
    ['Vestments / Cryptbloom / elite tectonic · 90+ Defence','These are late-game specialist sets: choose based on your combat style and encounter. Their value comes from strong style-specific effects, not simply their armour tier.'],
  ] as const;
  return <div className="content feature-page general-info-page"><section className="feature-heading"><div><p className="date-line">REFERENCE LIBRARY</p><h2>Good general information</h2><p>Useful consumables and equipment effects that are easy to overlook while building a self-sufficient Ironman account.</p></div></section><section className="general-callout panel"><Gem size={31}/><div><p className="eyebrow">How to use this page</p><h3>Keep supplies purposeful.</h3><p>Use this as a practical shortlist. Exact durations, charges and availability can change, so check the linked Wiki references before preparing a large batch.</p></div></section><InfoSection title="Het’s Oasis powders" eyebrow="Short-term power-ups" items={powders}/><InfoSection title="Incense sticks" eyebrow="Firemaking buffs" items={incense} note="One incense effect can run below 71 Firemaking; at 71 you can run two, and at 95 you can run three. Adding sticks builds potency and can extend the duration."/><InfoSection title="Other useful consumables" eyebrow="Prepare before the trip" items={consumables}/><InfoSection title="Relevant armour set effects" eyebrow="Mid to late game · 60+ Defence" items={armour} note="Not every armour set at this level has a meaningful set effect. These are the options where the effect or role is usually worth evaluating for Ironmen."/><p className="guide-credit"><BookOpen size={14}/> References: <a href="https://runescape.wiki/w/Update%3AIntroducing_Het%27s_Oasis" target="_blank" rel="noreferrer">Het’s Oasis powders</a>, <a href="https://runescape.wiki/w/Update%3AFiremaking_Week_%26_PvM_Revitalisation" target="_blank" rel="noreferrer">incense mechanics</a>, <a href="https://runescape.wiki/w/Barrows_equipment" target="_blank" rel="noreferrer">Barrows equipment</a>, and <a href="https://runescape.wiki/w/Crystal_equipment" target="_blank" rel="noreferrer">crystal equipment</a>.</p></div>;
}
