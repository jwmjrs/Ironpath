...
const nav = [[LayoutDashboard, 'Overview'], [Trophy, 'HiScores'], [Dice5, 'Task Generator'], [Boxes, 'Group Hub'], [CalendarCheck2, 'Repeatables'], [BookOpen, 'Ironman Guide'], [ListChecks, 'Efficient Progress'], [Gem, 'Good general information']] as const;
const themes = [
...
function HiScoresView({ result, setResult }: { result: HiscoreResult | null; setResult: (value: HiscoreResult | null) => void }) {
  ...
  return <div className="content feature-page">
    ...
    {result && <>
      ...
      <GroupDropLog group={result.group} players={result.players.map(player => player.name)} />
...
function GeneralInformationView() {
  const powders = [
    ['Powder of protection','Boosts the damage reduction from protection and deflection prayers. A good defensive option when learning combat.'],
    ['Powder of burials','Greatly increases Prayer experience from burying bones, making it a useful stockpile item for focused Prayer training.'],
    ['Powder of penance','Turns damage taken into Prayer restoration. Useful when prayer upkeep is the limiting resource.'],
    ['Powder of item protection','Protects an additional item on death. Carry it for dangerous activities where item loss is a concern.'],
    ['Powder of pulverising','Automatically grinds bones at the Ectofuntus, saving a repetitive processing step.'],
    ['Powder of defence','Strengthens defensive prayers for its duration. A situational layer of protection for tougher content.'],
  ] as const;
  const incense = [
    ['Ranarr incense','Adds Prayer bonus as its potency builds; a simple choice for prayer-heavy combat and Slayer.'],
    ['Marrentill incense','Reduces poison damage, helping on poison-heavy routes and encounters.'],
    ['Tarromin incense','Sends ashes from firemaking to the bank, which is useful when gathering large ash supplies.'],
    ['Guam incense','Can provide additional normal logs while woodcutting.'],
  ] as const;
  const consumables = [
    ['Supercompost and ultracompost','Protect crops and improve the value of farm runs. Keep a supply before committing to large herb or tree cycles.'],
    ['Juju farming potion','A high-value farming consumable that can increase herb yields; especially worthwhile for limited seeds.'],
    ['Overloads','Late-game combat potions that combine high stat boosts with a timed refresh. A major self-sufficiency milestone.'],
    ['Weapon poison','Adds passive damage when applicable. It is most valuable on targets that live long enough for repeated hits.'],
    ['Vulnerability bombs','Applies vulnerability in an area, making it a useful group support consumable for PvM.'],
    ['Adrenaline and replenishment potions','Situational combat supplies that help maintain ability rotations during difficult fights.'],
  ] as const;
  return <div className="content feature-page general-info-page"><section className="feature-heading"><div><p className="date-line">REFERENCE LIBRARY</p><h2>Good general information</h2><p>Useful consumables and equipment effects that are easy to overlook while building a self-sufficient Ironman account.</p></div></section><section className="general-callout panel"><Gem size={31}/><div><p className="eyebrow">How to use this page</p><h3>Keep supplies purposeful.</h3><p>Use this as a practical shortlist. Exact durations, charges and availability can change, so check the linked Wiki references before preparing a large batch.</p></div></section><InfoSection title="Het’s Oasis powders" eyebrow="Short-term power-ups" items={powders}/><InfoSection title="Incense sticks" eyebrow="Firemaking buffs" items={incense} note="One incense effect can run below 71 Firemaking; at 71 you can run two, and at 95 you can run three. Adding sticks builds potency and can extend the duration."/><InfoSection title="Other useful consumables" eyebrow="Prepare before the trip" items={consumables}/><ArmourEffectsSection/><p className="guide-credit"><BookOpen size={14}/> References: <a href="https://runescape.wiki/w/Update%3AIntroducing_Het%27s_Oasis" target="_blank" rel="noreferrer">Het’s Oasis powders</a>, <a href="https://runescape.wiki/w/Update%3AFiremaking_Week_%26_PvM_Revitalisation" target="_blank" rel="noreferrer">incense mechanics</a>, <a href="https://runescape.wiki/w/Barrows_equipment" target="_blank" rel="noreferrer">Barrows equipment</a>, <a href="https://runescape.wiki/w/Crystal_equipment" target="_blank" rel="noreferrer">crystal equipment</a>, and <a href="https://runescape.wiki/w/Masterwork_melee_equipment" target="_blank" rel="noreferrer">masterwork armour</a>.</p></div>;
}
...
function InfoSection({ title, eyebrow, items, note }:{ title:string; eyebrow:string; items:readonly (readonly [string,string])[]; note?:string }) {
  return <section className="general-info-section panel"><div className="panel-heading"><div><p className="eyebrow">{eyebrow}</p><h3>{title}</h3></div></div>{note && <p className="general-section-note">{note}</p>}<div className="general-info-grid">{items.map(([name, detail]) => <article key={name}><h4>{name}</h4><p>{detail}</p></article>)}</div></section>;
}

const armourEffects = [
  ['Ahrim the Blighted · 70 Defence','Magic Barrows set','A full set gives Blighted Aura: successful magic attacks can lower an enemy’s Strength by five levels. It is a niche defensive effect rather than a best-in-slot damage set.'],
  ['Dharok the Wretched · 70 Defence','Melee Barrows set','Wretched Strength raises maximum melee hit as your life points fall; the effect is strongest at very low life points. Best treated as a specialised risk/reward set.'],
  ['Guthan the Infested · 70 Defence','Melee Barrows set','Infestation can heal life points equal to damage dealt on a successful proc. This makes it valuable for long, lower-risk combat where saving food matters.'],
  ['Karil the Tainted · 70 Defence','Ranged Barrows set','A full set can lower an opponent’s Agility through successful ranged attacks. It is generally a utility or collection set, not a modern PvM damage choice.'],
  ['Torag the Corrupted · 70 Defence','Melee Barrows set','Corruption can reduce an opponent’s run energy. It has limited PvM value, but remains a complete defensive Barrows option.'],
  ['Verac the Defiled · 70 Defence','Melee Barrows set','The full set can bypass an opponent’s armour on successful procs. Its effect is mostly a legacy/niche option compared with modern ability-based combat gear.'],
  ['Crystal armour · 70 Defence','Ranged tank set','Its charge-based set effect improves defensive utility for ranged gameplay. Consider it when survival and ranged tanking matter more than pure damage.'],
  ['Anima Core armour · 80 Defence','Power armour sets','The Seren, Zaros and Sliske base sets are reliable non-degrading power armour. They do not depend on a full-set passive; their strength is accessible style coverage and upgrade potential.'],
  ['Trimmed masterwork melee · 92 Defence','Melee power armour','With at least three pieces, incoming damage is partly delayed over six seconds—10% per worn piece, up to 50% for all five. This makes large hits less spiky.'],
  ['Custom-fit trimmed masterwork · 92 Defence','Melee upkeep upgrade','Keeps the delayed-damage set effect while doubling charge life. It also does not degrade on relevant Slayer, Reaper, or Elite Dungeon targets, making it especially practical for Ironmen.'],
  ['Cryptbloom armour · 90 Defence','Magic tank armour','Nature’s Envoy grants stacking incoming magic and melee damage reduction as more pieces are worn. At five pieces it also provides a low-health fungal shield, making it a major learning and tanking set.'],
  ['Achto raid armour · 90 Defence','Raid tank armour','A defensive raid set built around shield and defensive ability use. Its full-set benefit is most meaningful in dedicated tank/support roles rather than routine damage roles.'],
  ['Vestments of Havoc · 95 Defence','Melee power armour','A late-game melee set designed for aggressive ability use and bleed-oriented combat. Its value is offensive pressure, so it is best where the encounter is already comfortable enough to trade defence for damage.'],
  ['Elite tectonic · 92 Defence','Magic power armour','High-tier magic power armour with strong offensive stats. It does not have a four-piece passive to manage; use it when raw magic damage is the goal.'],
] as const;

function ArmourEffectsSection() {
  return <section className="general-info-section panel armour-effects"><div className="panel-heading"><div><p className="eyebrow">Mid to late game · 60+ Defence</p><h3>Relevant armour effects</h3></div></div><p className="general-section-note">Open any entry for what the complete set does and when it is useful. This is a practical Ironman reference, not a replacement for the full equipment database.</p><div className="armour-effect-list">{armourEffects.map(([name, role, detail]) => <details key={name}><summary><span><strong>{name}</strong><small>{role}</small></span><ChevronRight size={16}/></summary><p>{detail}</p></details>)}</div></section>;
}
