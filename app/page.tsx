...
import { Archive, BookOpen, Boxes, CalendarCheck2, Check, ChevronRight, CircleDot, Clock3, Coins, Dice5, Dumbbell, ExternalLink, FlaskConical, Gem, LayoutDashboard, Leaf, ListChecks, Medal, RefreshCw, Search, Shield, Skull, Sparkles, Trophy, Users, Wrench } from 'lucide-react';
import { FormEvent, useEffect, useState } from 'react';
import progressData from './data/efficient-progress.json';
import { ironTasks, type IronTask } from './data/random-tasks';
import { archaeologyCollectors, type ArchaeologyCollector } from './data/archaeology-collectors';
import { dungeonBosses, dungeonPotions, dungeonPouchFamilies } from './data/dungeon-tools';
...
const themes = [
  ['classic','Classic'], ['necromancy','Necromantic'], ['wilderness','Wilderness'],
] as const;
  ...
  return <section className="ironpath-entry" aria-label="Ironpath introduction">
    ...
    <div className="entry-main">
      <div className="entry-copy">
        ...
        <div className="entry-logo-button entry-logo-static"><img src="/ironpath-banner-transparent-v1.png" alt="Ironpath"/></div>
...
type WikiCollection = { name:string; artefacts:string[]; reward:string; recurring:string };
type CollectorResult = { collector:ArchaeologyCollector; collections:WikiCollection[] };
type DungeonPouch = { family:string; pouch:string; charm:string; tertiary:string };
type BossDetail = { name:string; image:string|null; description:string; mechanics:string };

function wikiLink(title:string) { return `https://runescape.wiki/w/${encodeURIComponent(title.replaceAll(' ','_'))}`; }
...
function ArchaeologyCollectionsView() {
  ...
  useEffect(() => { try { setChecked(JSON.parse(window.localStorage.getItem('ironpath-archaeology-checks') || '{}')); } catch { setChecked({}); } },[]);
  function toggleArtefact(collection:string,artefact:string) { const key=`${collection}:${artefact}`; setChecked(current => { const next={...current,[key]:!current[key]}; window.localStorage.setItem('ironpath-archaeology-checks',JSON.stringify(next)); return next; }); }

  useEffect(() => {
    if (!selected) { setResult(null); return; }
    let cancelled=false;
    setLoading(true); setError('');
    fetch(`/api/archaeology-collections?collector=${encodeURIComponent(selected.name)}`,{cache:'no-store'})
      .then(async response => { const payload=await response.json() as CollectorResult & {error?:string}; if (!response.ok) throw new Error(payload.error || 'The collection could not be loaded.'); return payload; })
      .then(payload => { if (!cancelled) setResult(payload); })
      .catch(reason => { if (!cancelled) setError(reason instanceof Error ? reason.message : 'The collection could not be loaded.'); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled=true; };
  },[selected]);

  return <div className="content feature-page archaeology-page">
    <section className="feature-heading"><p className="eyebrow">Test Features</p><h2>Archaeology Collections</h2><p>Choose a collector to see their location, collection requirements and first-completion reward.</p></section>
    <section className="panel archaeology-selector"><div className="section-heading"><div><p className="eyebrow">Collector directory</p><h3>Who to visit</h3></div><a href="https://runescape.wiki/w/Collections" target="_blank" rel="noreferrer">Wiki reference <ExternalLink size={13}/></a></div><div className="archaeology-collector-grid">{archaeologyCollectors.map(collector => <button key={collector.name} className={selected?.name===collector.name ? 'archaeology-collector active' : 'archaeology-collector'} onClick={() => setSelected(collector)}><strong>{collector.name}</strong><span>{collector.location}</span><small>{collector.collections.length} collection{collector.collections.length===1?'':'s'}{collector.seasonal ? ' · seasonal' : ''}</small></button>)}</div></section>
    {selected && <section className="panel archaeology-detail"><div className="section-heading"><div><p className="eyebrow">Selected collector</p><h3>{selected.name}</h3><span>{selected.location}</span></div><a href={wikiLink(selected.name)} target="_blank" rel="noreferrer">Collector wiki <ExternalLink size={13}/></a></div>{loading && <p className="reference-status">Loading collection requirements from the RuneScape Wiki…</p>}{error && <p className="reference-status error">{error}</p>}{result && <div className="arch-collection-list">{result.collections.map(collection => { const done=collection.artefacts.filter(artefact => checked[`${collection.name}:${artefact}`]).length; return <details className="arch-collection" key={collection.name}><summary><span><strong>{collection.name}</strong><small>{done}/{collection.artefacts.length || 'Wiki'} checked</small></span><ChevronRight size={18}/></summary><div className="arch-collection-body"><div className="arch-reward"><span>Unique first-completion reward</span><strong>{collection.reward}</strong></div>{collection.recurring !== 'No unique reward listed' && <p className="arch-repeat">Repeat reward: {collection.recurring}</p>}{collection.artefacts.length > 0 ? <ul className="arch-artefacts">{collection.artefacts.map(artefact => <li className={checked[`${collection.name}:${artefact}`] ? 'checked' : ''} key={artefact}><button onClick={() => toggleArtefact(collection.name,artefact)} aria-label={`Mark ${artefact} ${checked[`${collection.name}:${artefact}`] ? 'not collected' : 'collected'}`}>{checked[`${collection.name}:${artefact}`] && <Check size={12}/>}</button><span>{artefact}</span></li>)}</ul> : <p className="arch-repeat">Open the Wiki entry to view this collection’s full artefact list.</p>}<a className="reference-link" href={wikiLink(collection.name)} target="_blank" rel="noreferrer">View collection on the Wiki <ExternalLink size={13}/></a></div></details>; })}</div>}</section>}
  </div>;
}
...
function DungeonToolsView() {
  ...
  return <div className="content feature-page dungeon-tools-page">
    ...
    <section className="panel dungeon-section"><div className="section-heading"><div><p className="eyebrow">Summoning pouches</p><h3>Charm and tertiary ingredients</h3></div><a href="https://runescape.wiki/w/Dungeoneering/Summoning" target="_blank" rel="noreferrer">Summoning wiki <ExternalLink size={13}/></a></div><div className="dungeon-pouch-grid">{dungeonPouchFamilies.map(([family,role,material]) => { const familyPouches=pouches.filter(pouch => pouch.family===family); return <details className="dungeon-pouch-family" key={family}><summary><span><strong>{family}</strong><small>{role} · usually {material}</small></span><ChevronRight size={18}/></summary><div>{loading ? <p className="reference-status">Loading Wiki recipes…</p> : familyPouches.length ? familyPouches.map(pouch => <div className="pouch-row" key={pouch.pouch}><strong>{pouch.pouch}</strong><span className={`charm-tag ${pouch.charm.toLowerCase().replace(' charm','')}`}>{pouch.charm} charm</span><span>{pouch.tertiary || 'Tertiary ingredient listed on Wiki'}</span></div>) : <p className="reference-status">Open the Wiki reference for this family’s recipes.</p>}</div></details>; })}</div></section>
    <section className="panel dungeon-section"><div className="section-heading"><div><p className="eyebrow">Dungeoneering bosses</p><h3>Bosses by floor theme</h3></div><a href="https://runescape.wiki/w/Dungeoneering/Bosses" target="_blank" rel="noreferrer">Boss wiki <ExternalLink size={13}/></a></div><p className="boss-help">Open a floor theme, then choose a boss to see its Wiki-backed encounter card.</p><div className="dungeon-boss-grid">{Object.entries(dungeonBosses).map(([theme,bosses]) => <details className="dungeon-boss-family" key={theme}><summary><span><strong>{theme}</strong><small>{bosses.length} bosses</small></span><Skull size={17}/></summary><ul>{bosses.map(name => <li key={name}><button className={boss===name ? 'boss-trigger active' : 'boss-trigger'} onClick={() => setBoss(current => current===name ? null : name)}>{name}<ChevronRight size={13}/></button></li>)}</ul></details>)}</div>{boss && <aside className="boss-detail-popover" id="boss-detail-popover"><button className="boss-detail-close" onClick={() => setBoss(null)} aria-label="Close boss details">×</button>{bossLoading && <p className="reference-status">Loading {boss} from the RuneScape Wiki…</p>}{bossDetail && <><div className="boss-detail-heading">{bossDetail.image && <img src={bossDetail.image} alt={bossDetail.name}/>}<div><p className="eyebrow">Boss encounter</p><h3>{bossDetail.name}</h3></div></div><p>{bossDetail.description}</p><div className="boss-mechanics"><strong>Mechanics</strong><p>{bossDetail.mechanics}</p></div><a className="reference-link" href={wikiLink(bossDetail.name)} target="_blank" rel="noreferrer">Read the full Wiki guide <ExternalLink size={13}/></a></>}</aside>}</section>
  </div>;
}
...
function HiScoresView({ result, setResult, workspace, setWorkspace, preferredMember, setPreferredMember }: { result: HiscoreResult | null; setResult: (value: HiscoreResult | null) => void; workspace:Workspace|null; setWorkspace:(value:Workspace|null)=>void; preferredMember:string; setPreferredMember:(name:string)=>void }) {
  ...
  async function joinPrivateGroupKey() {
    ...
    if (!id || !token) { setShareMessage('That does not look like a complete shared drop archive key.'); return; }
    setShareMessage('Connecting to the shared drop archive…');
    try {
      const response = await fetch('/api/workspace', { headers:{'x-ironpath-workspace':id,'x-ironpath-token':token} });
      const value = await response.json() as Omit<Workspace,'token'> & { error?:string };
      if (!response.ok) throw new Error(value.error || 'That shared drop archive key could not be verified.');
      const next = { ...value, token, data:{...emptyWorkspaceData,...value.data} };
      setWorkspace(next);
      window.localStorage.setItem('ironpath-workspace', JSON.stringify({ id, token }));
      setArchiveKey('');
      setJoinArchiveOpen(false);
      setShareMessage('Shared drop archive connected on this device.');
    } catch (reason) {
      setShareMessage(reason instanceof Error ? reason.message : 'Unable to connect to the shared drop archive.');
    }
  }
  ...
  return <div className="content feature-page repeatables-tracker">
    ...
    {result && <>
      ...
      <section className="panel activity-panel">
        ...
        {activityLoading ? <div className="small-empty"><RefreshCw className="spin" size={22}/><h3>Gathering group milestones</h3><p>Checking every member&apos;s public RuneMetrics activity log. Temporary failures are retried automatically.</p></div> : visibleActivities.length ? <div className="activity-feed">{visibleActivities.map((activity,index) => <article key={`${activity.player}-${activity.timestamp}-${index}`}><div className="activity-avatar character-chathead"><img src={characterAvatarUrl(activity.player)} alt="" onError={event => event.currentTarget.parentElement?.classList.add('avatar-unavailable')} /></div><div className="activity-copy"><div><strong>{activity.text}</strong><span>{activity.player}</span></div><p>{activity.details}</p></div><time dateTime={new Date(activity.timestamp).toISOString()}>{activity.date}</time></article>)}</div> : <div className="small-empty"><Clock3 size={22}/><h3>No public milestones found</h3><p>{activityMember === 'all' ? 'Members must set their RuneMetrics profile and online status to public for activities to appear.' : `No recent public activities were returned for ${activityMember}.`}</p></div>}
      </section>
      </div>
      <section className="panel dashboard-share-key"><div><p className="eyebrow">Shared drop archive</p><h3>{workspace ? 'Share your group’s item history' : 'Create or join a private drop archive'}</h3><p>{workspace ? 'Copy this key for trusted group members. It only connects everyone to the shared drop archive, so item history and past drops appear together.' : 'Create a key for your group, or join one shared by a trusted group member. Checklists, routes, and personal progress remain private to each device.'}</p></div><div className="dashboard-share-actions">{workspace ? <button className="primary-button" onClick={copyPrivateGroupKey}>Copy drop archive key</button> : <><button className="primary-button" onClick={createPrivateGroupKey}>Create and copy archive key</button><button className="secondary-button" onClick={() => { setArchiveKey(''); setJoinArchiveOpen(true); }}>Join archive</button></>}{shareMessage && <output>{shareMessage}</output>}</div></section>{joinArchiveOpen && <div className="archive-dialog-backdrop" role="presentation"><form className="archive-dialog panel" onSubmit={event => { event.preventDefault(); joinPrivateGroupKey(); }}><button type="button" className="archive-dialog-close" onClick={() => setJoinArchiveOpen(false)} aria-label="Close join archive dialog">×</button><p className="eyebrow">Shared drop archive</p><h3>Join your group archive</h3><p>Paste the key shared by a trusted group member. It only syncs your group&apos;s item history.</p><label className="field"><span>Group archive key</span><input value={archiveKey} onChange={event => setArchiveKey(event.target.value)} placeholder="Paste shared archive key" /></label><div className="archive-dialog-actions"><button type="button" className="secondary-button" onClick={() => setJoinArchiveOpen(false)}>Cancel</button><button className="primary-button">Join archive</button></div></form></div>}
    </>}
  </div>;
}
...
function archiveEntryDate(event:LoggedDrop) {
  ...
  return match ? `${match[1]}-${match[2]}-${match[3]} 00:00` : 'Date unavailable';
}
...
function characterAvatarUrl(name:string) { return `https://secure.runescape.com/m=avatar-rs/${encodeURIComponent(name)}/chat.png`; }
