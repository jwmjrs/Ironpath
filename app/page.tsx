...
function HiScoresView({ result, setResult, workspace, setWorkspace, preferredMember, setPreferredMember }: { result: HiscoreResult | null; setResult: (value: HiscoreResult | null) => void; workspace:Workspace|null; setWorkspace:(value:Workspace|null)=>void; preferredMember:string; setPreferredMember:(name:string)=>void }) {
  ...
  async function joinPrivateGroupKey() {
    ...
    if (!id || !token) { setShareMessage('Paste the complete shared drop archive key first.'); return; }
  ...
  return <div className="content feature-page repeatables-tracker">
    ...
    {result && <>
      ...
      <section className="panel dashboard-share-key"><div><p className="eyebrow">Shared drop archive</p><h3>{workspace ? 'Share your group’s item history' : 'Create or join a private drop archive'}</h3><p>{workspace ? 'Copy this key for trusted group members. It only connects everyone to the shared drop archive, so item history and past drops appear together.' : 'Create a key for your group, or paste one shared by a trusted group member. Checklists, routes, and personal progress remain private to each device.'}</p></div><div className="dashboard-share-actions">{workspace ? <button className="primary-button" onClick={copyPrivateGroupKey}>Copy drop archive key</button> : <><button className="primary-button" onClick={createPrivateGroupKey}>Create and copy archive key</button><div className="archive-key-join"><input value={archiveKey} onChange={event=>setArchiveKey(event.target.value)} placeholder="Paste shared archive key" aria-label="Shared drop archive key"/><button className="secondary-button" onClick={joinPrivateGroupKey}>Join archive</button></div></>}{shareMessage && <output>{shareMessage}</output>}</div></section>
    </>}
  </div>;
}
