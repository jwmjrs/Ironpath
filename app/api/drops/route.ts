...
function itemName(value:string) {
  return normalize(value)
    .replace(/,\s*the\s+[A-Za-z ]+?\s+pet(?:\s+drop)?(?:\s+at\s+[\d,]+\s+XP)?[.!]?$/iu,'')
    .replace(/\s+(?:drop|loot)$/iu,'')
    .trim();
}
function dropFromActivity(player:string, activity:Activity) {
  const title=(activity.text || '').replace(/\s+/gu,' ').trim();
  const detail=(activity.details || '').replace(/\s+/gu,' ').trim();
  const announcement=`${title} ${detail}`.trim();
  if (/(?:quest points|total levels|levelled|achievement|qualification)/iu.test(announcement)) return null;

  // RuneMetrics pet broadcasts use wording such as "received Herbert, the
  // Herblore pet drop" rather than the normal item-drop sentence.
  const pet=announcement.match(/(?:received|obtained|found|looted)\s+([A-Za-z][A-Za-z '-]{0,78}?),\s+the\s+[A-Za-z ]+?\s+pet(?:\s+drop)?\b/iu)
    || announcement.match(/(?:received|obtained|found|looted)\s+(?:a\s+)?(?:new\s+)?pet\s*:\s*([^.!]+?)(?:[.!]|$)/iu);
  if (pet) {
    const item=itemName(pet[1]); const occurredAt=timestamp(activity.date || '');
    return item ? { item,quantity:1,player,occurredAt,occurredDate:activity.date || '',activityText:title || detail } : null;
  }

  const match=title.match(/^(?:I\s+)?(?:received|obtained|found|looted)\s+(?:(\d[\d,]*)\s*(?:x|×)?\s+)?(?:an?\s+|the\s+)?(.+?)[.!]?$/iu);
  if (!match) return null;
  const item=itemName(match[2]); const occurredAt=timestamp(activity.date || '');
...
export async function GET(request:Request) {
  ...
  for (const player of players) { try { const response=await fetch(`https://apps.runescape.com/runemetrics/profile/profile?user=${encodeURIComponent(player)}&activities=20`,{headers:{'User-Agent':'Ironpath Group Ironman Companion'}}); const profile=await response.json() as { activities?:Activity[] }; for (const activity of profile.activities || []) { const drop=dropFromActivity(player,activity); if (!drop) continue; const existing=await env.DB.prepare('SELECT id FROM group_drop_archive WHERE group_key = ? AND player_name = ? AND occurred_at = ? AND activity_text = ? LIMIT 1').bind(key,drop.player,drop.occurredAt,drop.activityText).first(); if (existing) continue; const result=await env.DB.prepare('INSERT OR IGNORE INTO group_drop_archive (group_key,item_key,item_name,quantity,player_name,occurred_at,occurred_date,activity_text) VALUES (?,?,?,?,?,?,?,?)').bind(key,drop.item.toLocaleLowerCase(),drop.item,drop.quantity,drop.player,drop.occurredAt,drop.occurredDate,drop.activityText).run(); imported += result.meta.changes || 0; } } catch { /* Individual profiles may be unavailable. */ } }
  ...
  return Response.json({ group,imported,events:rows.results.map(row=>({item:itemName(row.item_name),quantity:row.quantity,player:row.player_name,timestamp:row.occurred_at,date:row.occurred_date,source:row.activity_text})) },{headers:{'Cache-Control':'private, no-store'}});
}
